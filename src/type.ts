...
export type StatusType = 'modified' | 'added' | 'deleted' | 'untracked' | 'other';

export interface ChangeWithDiffModified {
  uri: string;
  status: Extract<StatusType, 'modified'>;
  diff: string;
}

export interface ChangeWithDiffOther {
  uri: string;
  status: Exclude<StatusType, 'modified'>;
}

export type ChangeWithDiff = ChangeWithDiffModified | ChangeWithDiffOther;
